 ...
 * 
 * @param {Object} analysis - Análise fundamentalista completa
 * @returns {Object} Health Score com breakdown por pilar
 */
export function buildHealthScore(analysis) {
  if (!analysis || typeof analysis !== 'object') {
    throw new Error('Analysis inválida');
  }
...
function calculateRentabilidade(analysis) {
  ...
  const { roic, roe, margemEBITDA } = kpis;
...
function calculateCrescimento(analysis) {
  const kpis = analysis?.kpis || {};
  const { cagrReceita5a } = kpis;
  let score = 0;
  const rationale = [];
...
function calculateEstrutura(analysis) {
  const kpis = analysis?.kpis || {};
  const { divLiqSobreEBITDA, coberturaJuros } = kpis;
  let score = 0;
  const rationale = [];
...
function calculateGeracaoCaixa(analysis) {
  const kpis = analysis?.kpis || {};
  const { fcfSobreLL, dividendYield } = kpis;
  let score = 0;
  const rationale = [];
